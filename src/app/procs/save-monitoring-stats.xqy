...
for $item in $config-document/ML-mon-config:monitoring-config/ML-mon-config:monitoring-config-item
let $server-name := $item/ML-mon-config:server-name/text()
let $current-status := util:latest-status($server-name)
return
xdmp:node-insert-after($current-status/status/in-memory-size,xdmp:invoke("/app/procs/dynamic-monitoring.xqy",($constants:server-name-qname,$server-name)))
